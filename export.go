...
func (e *Exporter) optimisticExport(ctx context.Context) {
	rootKeyBytes := e.tree.ndb.nodeKey(GetRootKey(e.tree.version))
	...
	e.tree.root.traverse(e.tree, true, func(node *Node) bool {
		...
		exportNode := &ExportNode{
			Key:     e.tree.ndb.nodeKey(node.GetKey()), // TODO: How to get prefixed key so that import does not need to prefix?
			Value:   bytesCopy,
			Version: 0, // Version not used
			Height:  0, // Height not used
		}
